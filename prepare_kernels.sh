...
compile_kernels() {
  for kernel in $kernels; do
    ...
    make -C ./kernels/$kernel -j$(nproc) bzImage modules || { echo "Kernel $kernel compilation failed"; exit 1; }
    echo "Installing kernel $kernel modules"
    make -C ./kernels/$kernel -j$(nproc) INSTALL_MOD_PATH=./iso_root modules_install || { echo "Kernel $kernel module installation failed"; exit 1; }
    echo "Copying kernel $kernel bzImage"
    cp ./kernels/$kernel/arch/x86/boot/bzImage ./iso_root/boot/vmlinuz-$kernel || { echo "Kernel $kernel copy failed"; exit 1; }
    echo "Copying kernel $kernel config"
    cp ./kernels/$kernel/.config ./iso_root/boot/config-$kernel || { echo "Kernel $kernel copy failed"; exit 1; }
    echo "Copying kernel $kernel System.map"
    cp ./kernels/$kernel/System.map ./iso_root/boot/System.map-$kernel || { echo "Kernel $kernel copy failed"; exit 1; }
  done
}
